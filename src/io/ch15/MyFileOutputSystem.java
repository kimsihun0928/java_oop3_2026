...
public class MyFileOutputSystem {
    ...
    public static void main(String[] args) {
        ...
        // FileOutputStream fos = new FileOutputStream("output.txt", ture); --> Append 모드
        // 기존 파일에 내용이 있다면 true, 뒤에 이어쓰기, false -> 덮어쓰기

        try (FileOutputStream fos = new FileOutputStream("output.txt", false)) {
            // 문자열은 FileOutputStream 으로 직접 쓸 수 없음
            // 문자열을 -> byte 배열로 변환해서 넣어주어야 함
...
//            byte[] dataBytes = data.getBytes();
        ...
        // FileOutputsSystem 으로 바이트를 썼지만, 보여주는 방식은 에디터가 결정합니다.
